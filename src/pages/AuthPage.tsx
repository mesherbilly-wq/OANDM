...
function LoginForm({ onSwitch, initialEmail, lockEmail }: { onSwitch: () => void; initialEmail?: string; lockEmail?: boolean }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div>
        ...
        <input type="email" value={email} onChange={e => setEmail(e.target.value)} className={ic} placeholder="you@company.co.uk" required autoComplete="email" readOnly={lockEmail} />
      </div>
      <div>
      ...
      <p className="text-center text-sm text-slate-500">
        ...
        <button type="button" onClick={onSwitch} className="text-cyan-600 hover:text-cyan-700 font-semibold transition-colors">
          {lockEmail ? 'Create a password' : 'Register your company'}
        </button>
      </p>
    </form>
  );
...
function InviteRegisterForm({ email, onSwitch }: { email: string; onSwitch: () => void }) {
  const [password, setPassword] = useState('');
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div>
        <h3 className="text-base font-bold text-slate-900">Create your password</h3>
        <p className="text-xs text-slate-500 mt-0.5">Use the email this invite was sent to.</p>
      </div>
      <div>
        <label className={label}><Mail className="inline w-3.5 h-3.5 mr-1 -mt-0.5 text-slate-400" />Email Address</label>
        <input type="email" value={email} readOnly className={ic} />
      </div>
      <div>
        <label className={label}><Lock className="inline w-3.5 h-3.5 mr-1 -mt-0.5 text-slate-400" />Password</label>
        <div className="relative">
          <input type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
            className={`${ic} pr-11`} placeholder="Minimum 8 characters" required autoComplete="new-password" />
          <button type="button" onClick={() => setShowPwd(p => !p)}
      ...
      <div>
        <label className={label}><Lock className="inline w-3.5 h-3.5 mr-1 -mt-0.5 text-slate-400" />Confirm Password</label>
        <input type={showPwd ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
          className={ic} placeholder="Repeat your password" required autoComplete="new-password" />
      </div>
      {error && (
        <div className="flex items-center gap-2 bg-red-50 text-red-700 text-sm px-4 py-3 rounded-xl border border-red-200">
          <AlertCircle className="w-4 h-4 flex-shrink-0" />{error}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
            ...
            {inviteProjectName && (
              <p className="text-sm text-slate-600 mb-4">
                Invite to the O&amp;M pack for <span className="font-semibold text-slate-900">{inviteProjectName}</span>
            ...
            <div className="flex bg-slate-100 rounded-xl p-1 mb-7">
              {(['login', 'register'] as const).map(v => (
                <button key={v} onClick={() => setView(v)}
                  ...
                  {v === 'login' ? 'Sign In' : (inviteEmail ? 'Create password' : 'Register')}
                </button>
              ))}
            </div>

            {view === 'login'
              ? <LoginForm onSwitch={() => setView('register')} initialEmail={inviteEmail} lockEmail={Boolean(inviteEmail)} />
              : inviteEmail
                ? <InviteRegisterForm email={inviteEmail} onSwitch={() => setView('login')} />
